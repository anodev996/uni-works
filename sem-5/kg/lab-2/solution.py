...
# ----------------------------------------------------------------------
# Параметры расчета (задаются в коде)
# ----------------------------------------------------------------------
WIDTH = 900  # ширина изображения, px
HEIGHT = 520  # высота изображения, px
KMAX = 100  # максимальное число итераций
RMIN = 2.0  # радиус расходимости

# Границы расчета по варианту 2
X0, XN = -2.0, 0.8
Y0, YN = -1.0, 1.0


class MandelbrotApp:
    """GUI-приложение для построения фрактала Мандельброта."""
    ...
    def __init__(self, root):
        ...
        self.image_tk = None  # ссылка на изображение
    ...
    def _build_ui(self):
        ...
        # Информация о параметрах и границах
    ...
    def _compute_mandelbrot(self, width, height, x0, xn, y0, yn, kmax, rmin):
        """Возвращает матрицу M (height x width) с номерами итераций."""
    ...
    # ------------------------------------------------------------------
    # Раскраска: синий фрактал + синие ореолы
    # ------------------------------------------------------------------
    @staticmethod
    def _colorize(M, kmax):
        """
        Точки множества — чистый синий.
        Точки вне множества — плавный ореол: белый -> голубой -> синий.
        """
        h, w = M.shape
        img = np.full((h, w, 3), 255, dtype=np.uint8)  # белый фон

        # Внутренние точки (принадлежат множеству) — синие
        inside = M == kmax
        img[inside] = (0, 0, 255)
